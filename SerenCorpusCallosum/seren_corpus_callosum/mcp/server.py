"""
seren_corpus_callosum.mcp.server
════════════════════════════════
...
def mount_mcp_routes(app: FastAPI):
    """Mount the SCC MCP server onto an existing FastAPI app.

    Called from seren_corpus_callosum.app at startup IF the [mcp] extra is
    ...
    mount_path = os.environ.get("SEREN_SCC_MCP_MOUNT", "/mcp").rstrip("/")
    if not mount_path.startswith("/"):
        mount_path = "/" + mount_path

    federation = getattr(app.state, "federation", None)
    if federation is None:
        raise RuntimeError(
            "mount_mcp_routes called before app.state.federation was set. "
            "Mount inside the lifespan handler after the federation is built."
        )

    mcp = FastMCP("seren-corpus-callosum")
    register_tools(mcp, federation)
    ...
    # workstation reaching the box as `callosum-host:7423`. The rest of Seren is
    # trusted-LAN with the optional bearer token as the real gate, so we default
    # the host check OFF to match. Re-arm with SEREN_SCC_MCP_ALLOWED_HOSTS
    # (comma-sep) and optionally SEREN_SCC_MCP_ALLOWED_ORIGINS.
    ...
    logger.info("[seren-corpus-callosum] MCP server mounted at %s (%d tools)",
...
def _apply_transport_security(mcp) -> None:
    ...
    try:
        ...
        logger.info("[seren-corpus-callosum] transport_security module unavailable "
    ...
    allowed_hosts = _split("SEREN_SCC_MCP_ALLOWED_HOSTS")
    allowed_origins = _split("SEREN_SCC_MCP_ALLOWED_ORIGINS")
    ...
    if allowed_hosts or allowed_origins:
        ...
        logger.info("[seren-corpus-callosum] MCP host check ON; allowed_hosts=%s",
                    allowed_hosts)
    else:
        mcp.settings.transport_security = TransportSecuritySettings(
            enable_dns_rebinding_protection=False)
        logger.info("[seren-corpus-callosum] MCP host check OFF (trusted-LAN); set "
                    "SEREN_SCC_MCP_ALLOWED_HOSTS to enable an allowlist")
...
def _resolve_transport_app(mcp) -> object:
    ...
    for attr in ("streamable_http_app", "sse_app"):
        ...
        if callable(factory):
            logger.info("[seren-corpus-callosum] MCP transport: %s", attr)
            return factory()
    try:
