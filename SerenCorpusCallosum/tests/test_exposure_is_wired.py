...
seren-corpus-callosum's own main() calls it before the app is built, and that the
...
OPEN = 'server:\n  host: 0.0.0.0\n  port: 7423\n'
ALLOWED = 'server:\n  host: 0.0.0.0\n  port: 7423\n  allow_open_lan: true\n'
SCRUB = ("SEREN_SCC_ALLOW_OPEN_LAN", "SEREN_SCC_HOST", "SEREN_SCC_PORT", "SEREN_SCC_CONFIG",
         "SEREN_SCC_BEARER_TOKEN", "SEREN_SCC_BEARER_TOKEN_ENV", "SEREN_SCC_BEARER_TOKEN_KEYRING",
         "SEREN_SCC_TOKEN")
...
def _run(tmp_path, monkeypatch, yaml_text):
    ...
    holder = importlib.import_module("seren_corpus_callosum.__main__")
    monkeypatch.setattr(holder, "create_app",
                        lambda *a, **k: (_ for _ in ()).throw(_AppBuilt()))
    monkeypatch.setattr(sys, "argv", ["seren_corpus_callosum", "--config", str(cfg)])
    main_mod = importlib.import_module("seren_corpus_callosum.__main__")
    main_mod.main()
...
def test_the_env_override_lets_the_same_config_through(tmp_path, monkeypatch):
    ...
    for name in SCRUB:
        monkeypatch.delenv(name, raising=False)
    monkeypatch.setenv("SEREN_SCC_ALLOW_OPEN_LAN", "1")
    holder = importlib.import_module("seren_corpus_callosum.__main__")
    monkeypatch.setattr(holder, "create_app", lambda *a, **k: (_ for _ in ()).throw(_AppBuilt()))
    monkeypatch.setattr(sys, "argv", ["seren_corpus_callosum", "--config", str(cfg)])
    with pytest.raises(_AppBuilt):
        importlib.import_module("seren_corpus_callosum.__main__").main()
