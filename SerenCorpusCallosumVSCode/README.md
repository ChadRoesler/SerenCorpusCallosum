...
The family installer in [SerenSetupScripts](https://github.com/ChadRoesler/SerenSetupScripts) wires up a venv, the service, and a starter config that pre-fans your local Loci + Memory:
...
Open **`http://localhost:7423/viewer`**. It's a single-file dark UI with a violet accent — violet because it's the bridge between the coral right brain (Memory) and the cyan left brain (Loci):
...
| `search_loci` | keyed facts only | SerenLoci |
